...
tabs.forEach((tab) => {
  tab.addEventListener("click", () => {
    // Get the target element using the value of the "data-target" attribute
    const target = document.querySelector(tab.dataset.target);

    // Remove the "qualification__active" class from all tab content elements
    tabContents.forEach((tabContent) => {
      tabContent.classList.remove("qualification__active");
    });

    // Add the "qualification__active" class to the target tab content
...
contactForm.addEventListener("submit", (e) => {
  e.preventDefault;
  let res = window.alert("Your Message send Successfully");
